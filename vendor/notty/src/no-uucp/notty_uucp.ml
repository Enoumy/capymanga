(* Copyright (c) 2020 David Kaloper Meršinjak. All rights reserved. See
   LICENSE.md. *)
...
let find_i ~def k ((xs, _, _) as tab) =
  let rec go i j ((los, his, vs) as tab) (k : int) def =
    ...
    then def
      ...
      let x = (i + j) / 2 in
      if k < Array.unsafe_get los x
      then go i (x - 1) tab k def
      else if k > Array.unsafe_get his x
      then go (x + 1) j tab k def
      else Array.unsafe_get vs x)
  in
  go 0 (Array.length xs - 1) tab k def
;;

(* 12-6-6-bit (0xfff-0x3f-0x3f) trie, 3 levels, array-array-string. Root is
   variable; lower levels are either empty or complete. *)
let find_t ~def k tab =
  let k = if k > 0xd7ff then k - 0x800 else k in
  (* Pack to continuous range. *)
  let b0 = (k lsr 12) land 0xfff in
  if Array.length tab <= b0
  then def
  else (
    match Array.unsafe_get tab b0 with
    | [||] -> def
    | arr ->
      (match Array.unsafe_get arr ((k lsr 6) land 0x3f) with
       | "" -> def
       | str -> String.unsafe_get str (k land 0x3f) |> Char.code))
;;

(* We catch w = -1 and default to w = 1 to minimize the table. *)
let tty_width_hint u =
...
(* if w1 <> w2 then Format.printf "w: %a here: %d there: %d@." pp_u u w1
   w2; *)
(*     let gc1 = grapheme_cluster_boundary u *)
(*     and gc2 = Uucp.Break.Low.grapheme_cluster u in *)
(* if gc1 <> gc2 then Format.printf "gc: %a here: %d there: %d@." pp_u u gc1
   gc2; *)
