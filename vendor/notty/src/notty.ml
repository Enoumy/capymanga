...
module Text = struct
  ...
  let graphemes str =
    ...
    let rec f ((is, w) as acc) i evt =
      ...
      | `Boundary ->
        let is =
          match w with 0 -> is | 1 -> i :: is | _ -> i :: -1 :: is
        in
        f (is, 0) i `Await
    in
    let acc =
  ...
  let is_ascii_or_raise_ctrl s =
    ...
    let rec go s acc i n =
      ...
      then acc
        ...
        if is_C0 x
        then err_ctrl s.[i] s
        else if x = 0xc2 && n > 1 && is_C1 (s @! (i + 1))
        then err_ctrl s.[i + 1] s
        else go s (acc && is_ascii x) (i + 1) (n - 1))
    in
  ...
  let replicateu w u =
    if is_ctrl (Uchar.to_int u)
    then err_ctrl (Uchar.unsafe_to_char u) "<repeated character>"
    else if w < 1
    then empty
    else if is_ascii (Uchar.to_int u)
    then of_ascii (String.make w (Uchar.unsafe_to_char u))
    else
      of_unicode
      @@ Buffer.mkstring
      @@ fun buf ->
...
module A = struct
  ...
  let rgb ~r ~g ~b =
    ...
    then invalid_arg "Notty.A.rgb %d %d %d: channel out of range" r g b
    else 0x01000000 lor ((r * 36) + (g * 6) + b + 16)
  ;;

  let gray level =
    if level < 0 || level > 23
    then invalid_arg "Notty.A.gray %d: level out of range" level
    else 0x01000000 lor (level + 232)
  ;;

  let rgb_888 ~r ~g ~b =
    if r < 0 || g < 0 || b < 0 || r > 255 || g > 255 || b > 255
    then invalid_arg "Notty.A.rgb_888 %d %d %d: channel out of range" r g b
...
module I = struct
  ...
  let vcrop =
    let ctor top bottom img =
      let w = width img
      and h = height img - top - bottom in
  ...
  let text attr tx = if Text.is_empty tx then void 0 1 else Segment (attr, tx)
    ...
    let create () =
      ...
      let fmt =
        formatter_of_out_functions
          ...
          ; out_spaces =
              (fun w -> line := !line <|> char (top_a attr) ' ' w 1)
          ; out_indent =
              (fun w -> line := !line <|> char (top_a attr) ' ' w 1)
          }
      in
...
module Operation = struct
  ...
  let rec scan x w row i k =
    ...
    | Hcompose ((i1, i2), _) ->
      ...
      then scan (x - w1) w row i2 k
      else if x + w <= w1
      then scan x w row i1 k
      else scan x (w1 - x) row i1 @@ scan 0 (w - w1 + x) row i2 @@ k
    | Vcompose ((i1, i2), _) ->
      let h1 = height i1
      and h2 = height i2 in
      if row >= h1 + h2
      ...
      then scan x w (row - h1) i2 k
      else scan x w row i1 k
    | Zcompose ((i1, i2), _) ->
      let rec stitch x w row i = function
        ...
        | Text (a, t, ops) as opss ->
          ...
          if w1 >= w
          then opss
          else Text (a, t, stitch (x + w1) (w - w1) row i ops)
        | Skip (w1, ops) ->
          scan x w1 row i
          @@ if w1 >= w then ops else stitch (x + w1) (w - w1) row i ops
      in
      stitch x w row i2 @@ scan x w row i1 @@ k
    | Hcrop ((i, left, _), (w1, _)) ->
      if x >= w1
      then skip w k
      else if x + w <= w1
      then scan (x + left) w row i k
      else scan (x + left) (w1 - x) row i @@ skip (w - w1 + x) k
    | Vcrop ((i, top, _), (_, h1)) ->
  ...
  let ansi =
    ...
    ; cubcuf =
        (fun x b ->
          b <| "\x1b[";
          b <! abs x;
          b <. if x < 0 then 'D' else 'C')
    ; cuucud =
        (fun y b ->
          b <| "\x1b[";
          b <! abs y;
          b <. if y < 0 then 'A' else 'B')
    ; cr = (fun b -> b <| "\x1b[1G")
    ; clreol = (fun b -> b <| "\x1b[K")
    ; cursvis =
        (fun x b -> b <| if x then "\x1b[34h\x1b[?25h" else "\x1b[?25l")
    ; mouse =
        (fun x b ->
          b
          <|
          if x
          then "\x1b[?1000;1002;1005;1015;1006h"
...
module Render = struct
  ...
  let text_op cap buf a x =
    cap.sgr a buf;
    Text.to_buffer buf x
  ;;
...
module Unescape = struct
  ...
  let csi =
    ...
    and param prv p ps = function
      | x :: xs when btw x 0x30 0x39 ->
        param prv (Some ((get 0 p * 10) + x - 0x30)) ps xs
      | 0x3b :: xs -> param prv None (get 0 p :: ps) xs
      | xs -> code prv (List.rev (to_list p @ ps)) xs
    and code prv ps = function
      (* Conflate two classes because urxvt... *)
  ...
  let rec demux =
    ...
    | 0x1b :: 0x5b :: xs | 0x9b :: xs ->
      ...
      r :: demux xs
    | (0x1b :: 0x4f :: x :: xs | 0x8f :: x :: xs) when is_ascii x ->
      SS2 (chr x) :: demux xs
...
module Tmachine = struct
  ...
  let emit t op =
    if t.dead
    then invalid_arg "Notty: use of released terminal"
...
module Direct = struct
  ...
  and move_cursor buf cap cmd =
    match cmd with
    | `To (w, h) -> Cap.cursat0 cap w h buf
    | `Home -> cap.Cap.cr buf
    | `By (x, y) ->
      Cap.(
        if x <> 0 then cap.cubcuf x buf;
        if y <> 0 then cap.cuucud y buf)
  ;;
end
