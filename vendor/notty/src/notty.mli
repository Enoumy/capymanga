  ...
  (** An ineffable quality of light.
      ...
      {ul
       ...
       {- {e Extended 256-color palette.}
         ...
         terminals. Default palette tends to match {{:https://upload.wikimedia.org/wikipedia/commons/1/15/Xterm_256color_chart.svg} XTerm's}.
      }
       {- {e True color}
      ...
      Some of the technical and historical background can be found in {{:http://invisible-island.net/xterm/xterm.faq.html#problems_colors} XTerm's FAQ}.
...
module I : sig
  ...
  (** [string attr s] is an image containing text [s], styled with [attr].
      ...
      @raise Invalid_argument
        if [string] is not a valid UTF-8 sequence, or
...
[x.x] </> [yyyy] = [xyxy]
  ...
  (** [strf ?attr ?w:width format ...] pretty-prints like
      ...
      {{:http://caml.inria.fr/pub/docs/manual-ocaml/libref/Format.html#VALset_margin} [Format.set_margin]} for details. Defaults to an unspecified, large
      number.

      @raise Invalid_argument
        if the printing process attempts to directly
        output {{!ctrls} control characters}, by embedding them in [format] or a
        string printed with the [%s] conversion, for example.
        {{:http://caml.inria.fr/pub/docs/manual-ocaml/libref/Format.html#fpp} Formatted printing} is allowed. *)
  val strf
    :  ?attr:attr
    -> ?w:int
    -> ('a, Format.formatter, unit, image) format4
    -> 'a
...
(** {1:basics Basics}
    ...
    When in doubt, see
    {{:http://erratique.ch/software/uucp/doc/Uucp.Break.html#VALtty_width_hint} [Uucp.Break.tty_width_hint]}.

    Unicode has special interaction with {{!I.hcrop} horizontal cropping}:
    - Strings within images are cropped at {{:http://unicode.org/reports/tr29/#Grapheme_Cluster_Boundaries} grapheme
     ...
     {- {{!Render} Rendering} depends on image complexity.
       ...
       {[
         let wrap1 width img =
           ...
           in
           go img |> I.vcat |> I.hsnap ~align:`Left width
         ;;
       ]}
