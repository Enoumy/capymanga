...
external c_winsize : Unix.file_descr -> int = "caml_notty_winsize"
[@@noalloc]
...
module Term = struct
  module Winch = struct
    ...
    let add fd f =
      let n = !id in
      set_winch_handler (fun () -> Hashtbl.iter (fun _ f -> f ()) h)
      |> ignore;
      Hashtbl.add h n (fun () -> winsize fd |> iter f);
      incr id;
  ...
  let write t =
    ...
    Buffer.output_buffer t.output t.buf;
    flush t.output
  ;;
  ...
  let rec event = function
    ...
    | t ->
      Unix.(
        (try Input.event t.input with Unix_error (EINTR, _, _) -> event t))
  ;;
